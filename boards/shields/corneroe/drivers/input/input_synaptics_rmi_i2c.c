...
static void synaptics_poll(struct k_work *work) {
    ...
    if (ret != 0) {
        data->error_count++;
        if (data->error_count <= 5 || data->error_count % 100 == 0) {
            LOG_ERR("I2C read failed: %d (total errors: %u)",
                ret, data->error_count);
        }
    } else {
        data->success_count++;
        if (data->success_count <= 3) {
            LOG_INF("First successful read #%u: "
...
static int synaptics_init(const struct device *dev) {
    ...
    LOG_INF("Synaptics RMI I2C driver initialized");
    return 0;
}
